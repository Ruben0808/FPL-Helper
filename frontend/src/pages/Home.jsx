...
import { loginWithFPL, getGoogleSignInUrl, getLogoutUrl } from '../services/api';
...
export default function Home() {
  ...
  const { loadTeam, loading: dataLoading, googleUser } = useFPL();
  ...
  useEffect(() => {
    ...
    if (autoTeamId && !dataLoading) {
      loadTeam(autoTeamId).then(() => navigate('/my-team')).catch(() => {});
      setSearchParams({});
    } else if (authStatus === 'success') {
      // Signed in but no team saved yet — stay on page, show prompt
      setSearchParams({});
  ...
  const handleLoadById = async (e) => {
    ...
    setLoading(true);
    try {
      // Pass true to save team ID to Google account if signed in
      await loadTeam(teamId.trim(), true);
      navigate('/my-team');
    } catch (error) {
  ...
  const handleFPLLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const { entry } = await loginWithFPL(email.trim(), password);
      await loadTeam(entry, true);
      navigate('/my-team');
    } catch (error) {
      setErr(error.message);
    } finally {
    ...
    <div>
      ...
      <div className="bg-fpl-purple text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-14 pb-10 sm:py-20 text-center">
          ...
          <p className="text-lg sm:text-xl md:text-2xl text-gray-300 font-light mb-2">
          ...
          <div className="max-w-md mx-auto bg-white/10 border border-white/20 rounded-2xl p-5 backdrop-blur-sm">
            ...
            {/* Google Sign-In */}
            {googleUser ? (
              <div className="flex items-center justify-between gap-3 bg-white/10 border border-white/20 rounded-xl px-4 py-3 mb-4">
                <div className="flex items-center gap-3 min-w-0">
                  {googleUser.picture ? (
                    <img src={googleUser.picture} alt="" className="w-8 h-8 rounded-full shrink-0" referrerPolicy="no-referrer" />
                  ) : (
                    <div className="w-8 h-8 rounded-full bg-fpl-green flex items-center justify-center shrink-0">
                      <span className="text-fpl-purple font-black text-sm">{googleUser.name?.[0]}</span>
                    </div>
                  )}
                  <div className="min-w-0">
                    <p className="text-white text-sm font-semibold truncate">{googleUser.name}</p>
                    {googleUser.fplTeamId ? (
                      <p className="text-fpl-green text-xs">Team saved ✓</p>
                    ) : (
                      <p className="text-gray-400 text-xs">No team saved yet</p>
                    )}
                  </div>
                </div>
                <a href={getLogoutUrl()} className="text-xs text-gray-400 hover:text-white transition-colors shrink-0">
                  Sign out
                </a>
              </div>
            ) : (
              <a
                ...
                className="flex items-center justify-center gap-3 w-full bg-white text-gray-800 font-semibold text-sm py-3 rounded-xl hover:bg-gray-100 transition-all mb-4"
              >
                ...
                Sign in with Google to save your team
              </a>
            )}
